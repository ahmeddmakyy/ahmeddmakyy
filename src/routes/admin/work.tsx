...
export const Route = createFileRoute("/admin/work")({ component: WorkPage });

type Draft = Omit<WorkRecord, "id" | "sort_order">;
...
function WorkPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      setRows(await listWork());
      setError(null);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not load work cards.");
    }
  }, []);
  ...
  return (
    <>
      ...
      <div className="adm-actions" style={{ marginTop: 0, marginBottom: "1.25rem" }}>
        ...
        >
          {editing === "new" ? "Cancel" : "+ Add card"}
        </button>
        <span className="adm-who">{rows.length} cards</span>
      </div>
        ...
        <div key={row.id} className="adm-card">
          <div className="adm-card-head">
            <div style={{ marginInlineEnd: "auto", minWidth: "10rem" }}>
              <div className="adm-card-title">{row.title || "(untitled)"}</div>
              <div className="adm-card-sub">
                {row.tag_en} · {row.period_en}
              </div>
              <div
            ...
            <div style={{ display: "flex", gap: "0.375rem", flexWrap: "wrap" }}>
              <button
                ...
                title="Move up"
                disabled={i === 0}
                onClick={() => void run(() => swapWorkOrder(row, rows[i - 1]))}
              >
                ↑
              </button>
              <button
          ...
          {editing === row.id && (
            <WorkForm
              ...
              onCancel={() => setEditing(null)}
              onSave={async (d) => {
                await run(() => updateWork(row.id, d));
                setEditing(null);
              }}
              onDelete={async () => {
                if (!window.confirm(`Delete "${row.title}" permanently?`)) return;
                await run(() => deleteWork(row.id));
                setEditing(null);
              }}
            />
          )}
...
function WorkForm({
  ...
  return (
    ...
    >
      <Field label="Brand name (same in both languages)">
        <TextInput value={d.title} onChange={(v) => set("title", v)} placeholder="Demo Star" />
      </Field>
      ...
      <div className="adm-actions">
        <button
          ...
          onClick={() => void save()}
        >
          {busy ? "Saving…" : isEdit ? "Save changes" : "Create card"}
        </button>
